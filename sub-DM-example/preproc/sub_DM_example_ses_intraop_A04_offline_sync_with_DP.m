...
addpath('C:\Program Files\Brain-Modulation-Lab\NPMK\NPMK')
which openNEV

%% Defining paths
SUBJECT='DM10XX';
SESSION='intraop';
TASKS = {'sync', 'lombard', 'SMSL', 'ERNA', 'ECoGeStim'}; 
PATH_DATASET = 'Y:\DBS';
...
events_files = events_files(contains(events_files.filename, ['ses-' SESSION]), :); 
events_files = sortrows(events_files, 'starts'); 
events_files.id(:) = 1:height(events_files); 
...
% info_wav = info_wav_src_renamed; % if there is nothing in PATHS_AUDIO_SRC, uncomment this line & comment out the above block


% Saving files information table with OS times
info_orig = bml_annot_rowbind(info_ns5,info_nf3,info_nf6,info_nev, ...
                                info_neuroomega, ... % --- COMMENT OUT IF SUBJECT DOESN'T HAVE ALPHAOMEGA -----
                                info_wav,info_wav_src_renamed);
bml_annot_write_tsv(info_orig, [PATH_ANNOT filesep 'sub-' SUBJECT '_data-files-orig.tsv']);

%% ALIGNING TRELLIS FILES TO APPROX EVENT FILE TIMES (NO TIME WARPING--rigid body shift only)
...
for j = 1:height(events_files)
    ...
    events_tmp.trial_type = string(events_tmp.trial_type); % ERNA and ECoGeStim have character trial_types
...
for i=1:height(info_nev)    
    ...
    export_fig(gcf, ['./figures/A04_001_ripple2events_info-nev-idx-' int2str(i) '.png'])
...
for i=1:height(events_files)
    ...
    cfg.timetol=0.003;
    cfg.diagnostic_plot = true;
    [idxs_master_events,idxs_task_events, mean_sim]=bml_sync_match_events(cfg,master_events,task_events);    
    % eliminate first ~10 indices because they are almost always incorrect
    % since DP backtracking has little information in the 'top left' of the
    % distance matrix. 
    % These events' timestamp will be updated with a linear interoplation
    idxs_master_events(1:10) = [];
    idxs_task_events(1:10) = [];

    %linear warping as fallback strategy
    tbar = mean(task_events.starts(idxs_task_events),'omitnan');
    p = polyfit(task_events.starts(idxs_task_events) - tbar, master_events.starts(idxs_master_events) - tbar,1);
    ...
    s = sprintf('sync %s, delta_t=%f, mean_sim=%f, warpfactor=%f',events_files.filename{i}, p(2), mean_sim,  p(1))
    if cfg.diagnostic_plot
        title(gca,     split(s, '.tsv, '), 'interpreter', 'none')
        export_fig(gcf, ['./figures/A04_002_events2nev_events-file-' events_files.filename{i} '.png'])
    end
...
% note: order matters for info_wav_events because bml_annot_rowbind() will
% conform table2 to table1 columns. Prioritize task-related wav files first because they are more likely 
% to have associated Ripple data than sync wav files. 
for i=1:height(info_wav_events)
    ...
    cfg.weight_time_pre = 1; cfg.weight_time_post = 1; 
    cfg.weight_value_pre = 0.1; cfg.weight_value_post = 0.1; 
    cfg.sim_threshold = 0.8; 
    [i_sync_wav] = bml_sync_digital(cfg,master_trig, wav_events); % returns empty table if no matching events are found
    ...
    sync_wav = bml_annot_rowbind(sync_wav, i_sync_wav);
%     hF = [hF; i_hF];
    drawnow
end
...
for i=1:height(info_neuroomega_events)
    ...
    [i_sync_ao, i_hF] = bml_sync_digital(cfg,master_trig,i_slave_events); % should this be master_events? 
