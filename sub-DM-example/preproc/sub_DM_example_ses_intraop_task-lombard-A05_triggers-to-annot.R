...
annot_trials <- events_trig_trial %>%
  ...
  group_by(sub,ses,task,run_id,trial_id,trig_noise, time_col_name) %>%
  filter(n()==1) %>% # added LB 2024 02 27 to remove possible duplicates in events--happens VERY rarely
