...
rmpath('C:\Program Files\LeadDBS_Classic\leaddbs\tools\MER');
...
options.patientname = [subject filesep leaddbs_folder_name];
...
atlas_idx = find(strcmp(target, atlas_names), 1);
assert(~isempty(atlas_idx))

h = figure; hold on;
% plot STNs
for side=1:2
    Hp = patch('vertices',atlases.roi{atlas_idx,side}.fv.vertices,'faces',atlases.roi{atlas_idx,side}.fv.faces,...
...
% save figures
mkdir([subject filesep leaddbs_folder_name filesep 'figures']); 
saveas(h, [subject filesep leaddbs_folder_name filesep 'figures', filesep subject, '_MERlocs.fig'], 'fig');
saveas(h, [subject filesep leaddbs_folder_name filesep 'figures', filesep subject, '_MERlocs.pdf'], 'pdf');
