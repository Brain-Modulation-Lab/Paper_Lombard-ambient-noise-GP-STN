%% To do 
% - compare DLPFC with and without loudness
% - scan wider range of lambdas
% - for SU data--can we decode? 
%% Basic setup 
clc;
clear all;
close all;
...
CHANNEL_SUBSET = 'BG-STN';      % e.g., 'BG', 'PMC', 'IPC-TPOJ-AAC-LTC', 
CTX_FLAG = false;           % Is this cortex-specific analysis?
loudness_lvls = 'all'; % all, lowhalf-loudhalf
TASK = 'lombard';
level = 'sentence';

% Create log_hash
% 202505201517 updated lambda range 
log_hash = ['202505301102' '-' MODALITY '-' CHANNEL_SUBSET '_loudness-' loudness_lvls '_cofit-lmtime-noisetype']
% assert(0, 'chekc the above line--do you want artificial inflation of the effect? ')
...
switch MODALITY
    case 'SU'
        PATH_ANALYSIS = fullfile(PATH_GROUPANALYSES, 'task-lombard', '20231122-firing-rate_rep', 'latane');
        PATH_DATA_INPUT = fullfile(PATH_ANALYSIS, 'data', ['dpca_tensor_timewarped_cofit-lmtime-noisetype_loudness-' loudness_lvls '_202505261301.h5'] );
        
         % '_loudness-lowhalf-loudhalf_202505141138.h5'
         % -all_202505141137.h5
         % dpca_tensor_timewarped_NO-LMTIME_loudness-all_202505211242.h5
         % _cofit-lmtime-noisetype_loudness-all_202505261301.h5
        PATH_FIGUREOUTPUT = fullfile(PATH_ANALYSIS, 'results', 'figures');
        PATH_RESULTSOUTPUT = fullfile(PATH_ANALYSIS, 'results', 'output');
        PATH_RESULTSLOG = fullfile(PATH_ANALYSIS, 'results', 'log');
        PATH_ANNOTOUTPUT = fullfile(PATH_ANALYSIS, 'annot');
    ...
    case 'LFP'
        ...
        if CTX_FLAG
            ...
            PATH_DATA_INPUT = fullfile(PATH_ANALYSIS, 'data', ['tlock-audio-on_chs-' CHANNEL_SUBSET], ... 
                                        ['dpca_tensor_timewarped_loudness-' loudness_lvls '_202505142351.h5']); 
            % _202505141346.h5 for IPC-TPOJ-AAC
            % _loudness-all_202505141310.h5
            % 'loudness-lowhalf-loudhalf_202505142333.h5' for PMC
            % _loudness-all_202505142351.h5 PMC
            % _loudness-lowhalf-loudhalf_202505191757.h5 for DLPFC
            % _loudness-all_202505191634.h5 for DLPFC

        else
            % Subcortical (BG) or named group
            if contains(CHANNEL_SUBSET, 'BG')
                ...
                PATH_DATA_INPUT = fullfile(PATH_ANALYSIS, 'data', 'tlock-audio-on_chs-macro-ecogL-ecogL-ecogL-audio-envaudio', ...
                    ['dpca_tensor_timewarped_cofit-lmtime-noisetype_loudness-' loudness_lvls '_202505261341.h5']);
            % dpca_tensor_timewarped_loudness-all_202505141222.h5
            % '_202505141310.h5'
            %  dpca_tensor_timewarped_loudness-lowhalf-loudhalf_202505141221.h5
            % _cofit-lmtime-noisetype_loudness-all_202505261341.h5
            else
...
Xmeta.target = h5read(PATH_DATA_INPUT, '/target');


idxs = true(length(Xmeta.target), 1); 
if CHANNEL_SUBSET=="BG-STN"
    idxs = Xmeta.target=="STN";
elseif CHANNEL_SUBSET=="BG-GPi"
    idxs = Xmeta.target=="GPi";
end
warning(sprintf('Subsetting data: reducing %d neurons/lfps to %d\n', length(idxs), sum(idxs)))
X = X(idxs, :, :, :, :);
X_ntrials = X_ntrials(idxs, :, :);


% assert(0, 'inflating NOISE efffect')
% x1 = X(:, 1, :, :); x2 = X(:, 2, :, :); 
% figure; histogram(x1); hold on; histogram(x2)
% mean(x1(:),'omitnan')
% mean(x2(:),'omitnan')
% X(:, 2, :, :) = X(:, 2, :, :) + 2;
...
Xavg  = mean(X, length(size(X)), 'omitnan'); % average over trials
...
if size(X, 3)==1
    ...
    warning('SQUEEZING DATA and updating marginalizatins.')
...
%     X_ntrials = repmat(X_ntrials, [1 1 2]); 
    ...
    X = squeeze(X); 
    Xavg  = squeeze(Xavg); % average over trials
    X_ntrials = squeeze(X_ntrials); 

    combinedParams = {{1, [1 2]}, {2}};
    decodingClasses = {[1; 2], []};
    %decodingClasses = {[1; 2], [1 ;2], [], [1 ;2]};
    margNames = {'NOISE',     'Independent'};
    margColours = [CLRS.NOISE; CLRS.control];
    IDX_TIME_MARG = 2; 
end
...
ich = 15; % 40 DM1015, micro_Lc, we know this to be active 
x = squeeze(X(ich, :, 1, :, :)); 
x = mean(x, 3, 'omitnan'); 
...
elseif length(size(X))==4
    ...
    for n = 1:size(X,1)
        for s = 1:size(X,2)
            assert(isempty(find(isnan(X(n,s,:,1:trialNum_all(n,s))), 1)), 'Something is wrong!')
        end
    end
else assert(0, 'X data dims not recognized') 
end
...
if ~regularize
    ...
    dpca_plot(Xavg, W, V, @dpca_plot_default, ...
        ...
        'timeMarginalization', IDX_TIME_MARG, ...
        'legendSubplot', 16);
 
else
    mkdir(PATH_FIGUREOUTPUT); 
    lams = logspace(-4, 1, 20); % 1e-07 * 1.5.^(0:25)
    optimalLambdas_filename = fullfile(PATH_FIGUREOUTPUT, ['A06_optimalLambdas_' log_hash '.mat']); 
    if ~isfile(optimalLambdas_filename)
        optimalLambda = dpca_optimizeLambda(Xavg, X, trialNum_all, ...
            ...
            'filename', optimalLambdas_filename);
    else
        load(optimalLambdas_filename)
    end
...
exportgraphics(gcf(),fullfile(PATH_FIGUREOUTPUT, ['A06_dpcaplot_' log_hash '.pdf']),'ContentType','vector')
...
save(fullfile(PATH_FIGUREOUTPUT, ['A06_dpca_decomposition_' log_hash '.mat']),'dPCA')
% ld = load(fullfile(PATH_FIGUREOUTPUT,'A06_dPCA_decomposition.mat'))

%% Run decoding

accuracy_filename = fullfile(PATH_FIGUREOUTPUT,  ['A06_classification_accuracy_' log_hash '.mat']);

if ~isfile(accuracy_filename)
...
%     %dpca_classificationPlot(accuracy, [], accuracyShuffle, [], decodingClasses)
    ...
    %%
    componentsSignif = dpca_signifComponents(accuracy, accuracyShuffle, whichMarg, 'minChunk', 5);
...
exportgraphics(gcf(),fullfile(PATH_FIGUREOUTPUT, ['A06_dpcaplot-with-sig_' log_hash '.pdf']),'ContentType','vector')
...
NeuronEncoding_TimeCourse = V(:,find(whichMarg == 2 ,1)); % get the first one

figure; tiledlayout(2, 1); nexttile()
histogram(NeuronEncoding_Lombard, 'BinWidth', 0.01, 'FaceColor', 'k', 'EdgeColor', 'w');  nexttile()
histogram(NeuronEncoding_TimeCourse,'BinWidth', 0.01, 'FaceColor', 'k', 'EdgeColor', 'w');  
linkaxes(findobj(gcf(), 'Type', 'axes'))




figure; set(gcf,'Position',[ 1171        1063         389         275])

DotProduct = dot(NeuronEncoding_Lombard,NeuronEncoding_TimeCourse);
% figure
% scatter(NeuronEncoding_Lombard,NeuronEncoding_TimeCourse)
[tauK, psp] = corr(NeuronEncoding_Lombard,NeuronEncoding_TimeCourse, 'type', 'Kendall');
CosTheta = max(min(DotProduct/(norm(NeuronEncoding_Lombard)*norm(NeuronEncoding_TimeCourse)),1),-1);
ThetaInDegrees = real(acosd(CosTheta));
isSign = abs(DotProduct) > 3.3/sqrt(size(NeuronEncoding_TimeCourse,1)) & psp<0.001;

scatter(NeuronEncoding_Lombard,NeuronEncoding_TimeCourse,15,'k', 'filled')
xlabel('dPCA weight_{NOISE}')
ylabel('dPCA weight_{epoch}')
title(['\theta = ', sprintf('%1.2f',ThetaInDegrees), '° \tau = ', sprintf('%1.2f',tauK), ' (p = ',sprintf('%1.2f',psp),')']);%1.2f (p = %1.2f)',,tauK,psp),'Interpreter','latex')

exportgraphics(gcf(),fullfile(PATH_FIGUREOUTPUT, ['A06_03_encoding-scatter_' log_hash '.pdf']),'ContentType','vector')
...
maxline = prctile(squeeze(accuracyShuffle(1,:,:,1)),100,2)'; 
plot(time,maxline,'k--', 'linewidth',1,'color','k'); % visualize the 'max'
...
exportgraphics(gcf(),fullfile(PATH_FIGUREOUTPUT,  ['A06_02_decoding_' log_hash '.pdf']),'ContentType','vector')
...
box off
nexttile(3)
pd = fitdist(NeuronEncoding_TimeCourse,'Normal ');
x = -.5 : .01 : .5;
y = pdf(pd,x);
plot(x,y,'LineWidth',2,'color','k')
xlabel('dPCA weight_{Time}')
ylabel('pdf')
xlim([-.5 .5])
box off
